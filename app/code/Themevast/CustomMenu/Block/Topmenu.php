<?php
namespace Themevast\CustomMenu\Block;
...
class Topmenu extends \Magento\Framework\View\Element\Template
{
    ...
    public function __construct(
        ...
        \Themevast\CustomMenu\Helper\Data $helper,
    ...
    public function getSubmenuItemsHtml($children, $level = 1, $max_level = 0, $column_width=12, $menu_type = 'fullwidth', $columns = null)
    {
        ...
        if(!$max_level || ($max_level && $max_level == 0) || ($max_level && $max_level > 0 && $max_level-1 >= $level)) {
            ...
            foreach($children as $child) {
                ...
                $tv_menu_hide_item = $cat_model->getData('tv_menu_hide_item');
                
                if (!$tv_menu_hide_item) {
                    $sub_children = $this->getActiveChildCategories($child);
                    
                    $tv_menu_cat_label = $cat_model->getData('tv_menu_cat_label');
                    $tv_menu_icon_img = $cat_model->getData('tv_menu_icon_img');
                    $tv_menu_font_icon = $cat_model->getData('tv_menu_font_icon');
                    ...
                    if($level == 1 && $tv_menu_icon_img) {
                        $html .= '<div class="menu-thumb-img"><a class="menu-thumb-link" href="'.$this->_categoryHelper->getCategoryUrl($child).'"><img src="' . $this->_helper->getBaseUrl().'catalog/category/' . $tv_menu_icon_img . '" alt="'.$child->getName().'"/></a></div>';
                    }
                    $currentCatId = '';
                    if($this->getCurrentCategoryOb()){
                        $currentCatId=$this->getCurrentCategoryOb()->getId();
                    }
                    $activeClass='';
                    if($currentCatId==$child->getId()){
                        $activeClass='actived';
                    }
                    $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($child).'" class="'.$activeClass.'">';
                    if ($level > 1 && $tv_menu_icon_img)
                        $html .= '<img class="menu-thumb-icon" src="' . $this->_helper->getBaseUrl().'catalog/category/' . $tv_menu_icon_img . '" alt="'.$child->getName().'"/>';
                    elseif($tv_menu_font_icon)
                        $html .= '<em class="menu-thumb-icon '.$tv_menu_font_icon.'"></em>';
                    $html .= '<span>'.$child->getName();
                    if($tv_menu_cat_label)
                        $html .= '<span class="cat-label cat-label-'.$tv_menu_cat_label.'">'.$this->_custommenuConfig['cat_labels'][$tv_menu_cat_label].'</span>';
                    $html .= '</span></a>';
                    if(count($sub_children) > 0) {
    ...
    public function getCustomMenuHtml()
    {
        ...
        if($this->_custommenuConfig['general']['show_home']==1){
            $activeHome = '';
            if($this->isHomePage()){
                $activeHome='actived';
            }
            $linkHome = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_LINK);
            $html .= '<li class="ui-menu-item level0 '.$activeHome.'"><a class="level-top" href="'.$linkHome.'"><span>'.__('Home').'</span></a></li>';
        }
        foreach($categories as $category) {
            ...
            if(!$tv_menu_hide_item) {
                ...
                $tv_menu_icon_img = $cat_model->getData('tv_menu_icon_img');
                $tv_menu_font_icon = $cat_model->getData('tv_menu_font_icon');
                $tv_menu_cat_columns = $cat_model->getData('tv_menu_cat_columns');
                $tv_menu_float_type = $cat_model->getData('tv_menu_float_type');
                
                if(!$tv_menu_cat_columns){
                    $tv_menu_cat_columns = 4;
                }
                
                $menu_type = $cat_model->getData('tv_menu_type');
                if(!$menu_type)
                ...
                $tv_menu_static_width = $cat_model->getData('tv_menu_static_width');
                if($menu_type=="staticwidth" && $tv_menu_static_width)
                    $custom_style = ' style="width: '.$tv_menu_static_width.';"';
                    
                $item_class = 'level0 ';
                $item_class .= $menu_type.' ';
                
                $menu_top_content = $cat_model->getData('tv_menu_block_top_content');
                $menu_left_content = $cat_model->getData('tv_menu_block_left_content');
                $menu_left_width = $cat_model->getData('tv_menu_block_left_width');
                if(!$menu_left_content || !$menu_left_width)
                    $menu_left_width = 0;
                $menu_right_content = $cat_model->getData('tv_menu_block_right_content');
                $menu_right_width = $cat_model->getData('tv_menu_block_right_width');
                if(!$menu_right_content || !$menu_right_width)
                    $menu_right_width = 0;
                $menu_bottom_content = $cat_model->getData('tv_menu_block_bottom_content');
                if($tv_menu_float_type)
                    $tv_menu_float_type = 'fl-'.$tv_menu_float_type.' ';
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content)))
                    $item_class .= 'parent ';
                $html .= '<li class="ui-menu-item '.$item_class.$tv_menu_float_type.'">';
                if(count($children) > 0) {
                    $html .= '<div class="open-children-toggle"></div>';
                }
                $currentCatId = '';
                ...
                $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($category).'" class="level-top '.$activeClass.'">';
                if ($tv_menu_icon_img)
                    $html .= '<img class="menu-thumb-icon" src="' . $this->_helper->getBaseUrl().'catalog/category/' . $tv_menu_icon_img . '" alt="'.$category->getName().'"/>';
                elseif($tv_menu_font_icon)
                    $html .= '<em class="menu-thumb-icon '.$tv_menu_font_icon.'"></em>';
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content))) {
                    ...
                    if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_left_content || $menu_right_content))) {
                        ...
                        $html .= $this->getSubmenuItemsHtml($children, 1, $max_level, 12-$menu_left_width-$menu_right_width, $menu_type, $tv_menu_cat_columns);
    ...
    public function getCustomTopMenuResponsive()
    {
        $html = '';
        ...
        foreach($categories as $category) {
            ...
            if(!$tv_menu_hide_item) {
               ...
               $currentCatId = '';
                ...
                if(count($children) > 0 )
                    $item_class .= 'parent ';
                $html .= '<li class="ui-menu-item '.$item_class.' '.$activeClass.'">';
