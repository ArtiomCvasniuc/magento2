    ...
    public function execute() {
        ...
        foreach ( $attributes as $attribute ) {
            ...
            if (! $attribute->getApplyTo () || count ( array_diff ( $types, $attribute->getApplyTo () ) ) === 0) {
                ...
                $output .= '<li><input id="' . $attribute->getAttributeCode () . '" ' . $checked . ' name="attributes[' . $attribute->getAttributeCode () . ']" value="' . $attribute->getFrontendLabel () . '"
  title="' . $attribute->getFrontendLabel () . '" 
  class="attribute-checkbox validate-one-required-by-name" type="checkbox">';
                $output .= '<input type="hidden" name="attribute_ids[' . $attribute->getAttributeCode () . ']" value="' . $attribute->getAttributeId () . '" />';

                $output .= '<label for="' . $attribute->getAttributeCode () . '">' . $attribute->getFrontendLabel () . '</label></li>';
            }
        }
        $output .= '</ul>';
